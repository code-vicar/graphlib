(function () {
    ...
    var EdgeFactory = require('./edge');

    var Graph = function Graph(nvertices, opts) {
        var graph = this;

        // unwrap the options
        var defaultEdge = EdgeFactory.defineEdge();
        ...
        if (isArray(nvertices)) {
            ...
            console.warn('Graph expected first argument to be an array of vertices');
        }
        ...
        var privateInsertEdge = function privateInsertEdge(x, y, directed, edgeArgs) {
            ...
            if (!optsEdgeClass) {
                listX.push(new defaultEdge(y.id));
            } else {
